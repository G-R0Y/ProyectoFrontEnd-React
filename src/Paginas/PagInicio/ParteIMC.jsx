...
function ParteIMC() {
  ...
  const resultIMC = (datos) => {
    ...
    if (datos.altura <= 0 || datos.peso <= 0) {
      ...
      setValor({ imc: 0, display: "none" });
    } else {
      if (valorIMC < 18.5) {
        setValor({ imc: 2, display: "block" });
      } else if (valorIMC >= 18.5 && valorIMC < 25) {
        setValor({ imc: 3, display: "block" });
      } else if (valorIMC >= 25 && valorIMC < 30) {
        setValor({ imc: 4, display: "block" });
      } else if (valorIMC >= 30 && valorIMC < 35) {
        setValor({ imc: 5, display: "block" });
      } else if (valorIMC >= 35 && valorIMC < 40) {
        setValor({ imc: 6, display: "block" });
      } else if (valorIMC > 40) {
        setValor({ imc: 7, display: "block" });
      }
    }
  };
  ...
  return (
    <div className="container-fluid ParteIMC">
      <div className="row justify-content-evenly">
        <CalcularIMC resultIMC={resultIMC} />
